...
Este projeto é uma ferramenta de alta performance desenvolvida para gerenciar metadados de arquivos de áudio, com foco em sincronização de letras temporizadas (`.lrc`) e conversão otimizada de áudios de alta fidelidade **FLAC** para o codec **Opus (OGG)**.
...
*   **Pipeline de Metadados via Arquivos `.meta`**:
    Para contornar o limite de tamanho de argumentos do terminal do Windows ao lidar com metadados extensos (como letras com milhares de caracteres), o sistema escreve dinamicamente as tags Vorbis e a letra temporizada (lida e decodificada de arquivos `.lrc`) em um arquivo temporário no formato de metadados do FFmpeg (`ffmetadata`). Esse arquivo é limpo de forma garantida (`finally`) após a conclusão da conversão.
...
*   **Parsing Binário no Navegador (`flac.ts`)**:
    Analisa os bytes de arquivos FLAC selecionados pelo usuário para decodificar os blocos de metadados e extrair os comentários Vorbis diretamente na memória.
